...
//			startActivity(target);
			...
			File f = null;
			long newStatus = -1;
			newStatus = sendReport(serverUrl, reportJsonString);
			Log.i(TAG,"newStatus="+newStatus);
			if(newStatus == 0){
				f = new File("/sdcard/a.txt");
				try {
					f.createNewFile();
					Log.i(TAG,"new file successfully");
	...
	public long sendReport(String url , NameValuePair reportStr) {
...
//			conn.setRequestProperty("Client-Name", "ivi_collect");
			...
			case 200://HTTP status code, 200: OK 
				...
				id = Long.parseLong(jsonObject.getString(Util.JSON.STATUS));
